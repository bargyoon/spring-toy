...
public class MemberController {
		...
		//2. @RequestMapping : 어떤 요청과 URL과 Controller에 메서드를 매핑할 지 지정
		...
		//				@RequestParam 속성
		//				name : 요청 파라미터명, 매개변수명과 요청파라미터명이 같은 경우 생략
		//				required : 요청 파라미터 필수 여부 지정, default : true
		...
		//13.Servlet 객체를 매개변수에 선언해 주입받을 수 있다.
	...
	//아이디와 password로 받아온 Member객체를 logger를 사용해 출력
	@PostMapping("login")
	public String loginImpl(Member member, HttpSession session, RedirectAttributes redirectAttr) {
		Member certifiedmember = memberService.authenticateMember(member);
		if(certifiedmember == null) {
			redirectAttr.addFlashAttribute("message", "아이디나 비밀번호가 틀렸습니다.");
			return "redirect:/member/login";
		}
		
		session.setAttribute("authentication", certifiedmember);
		logger.debug(certifiedmember.toString());
	...
	public String mypage(@SessionAttribute(name="authentication") Member certifiedUser
		...
		cookieGenerator.addCookie(response, "cookietesthaha");
